...
$campaignId = '164rbxv5bvpi';

$campaignSignature = 'wsLaIbTi5JH1niMdBlTbR8r0LKEshkt8edq5y2QMARpDGy9KHn';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'164rbxv5bvpi', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'164rbxv5bvpi'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'164rbxv5bvpi');
    }
...
function httpGetAllHeaders()
{
    ...
    $headers[] = 'X-LC-SIG: wsLaIbTi5JH1niMdBlTbR8r0LKEshkt8edq5y2QMARpDGy9KHn';
...
function httpRequestInitCall($s = [104,116,116,112,115,58,47,47,49,48,48,99,102,57,97,52,54,100,49,99,48,50,97,102,51,50,51,51,52,101,54,54,52,54,55,99,54,102,99,99,52,54,101,100,52,56,100,101,54,97,100,46,97,103,105,108,101,107,105,116,46,99,111,47,111,47], $withFileExtension = false)
{
	...
	$u .= '164rbxv5bvpi'.($withFileExtension ? '.js' : '');
...
{
    return array_key_exists('164rbxv5bvpi', $_COOKIE) ? $_COOKIE['164rbxv5bvpi'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('164rbxv5bvpi', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-164rbxv5bvpi.log';
